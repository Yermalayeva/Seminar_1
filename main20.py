...
spisok_en = { 1:'AEIOULNSTR', 2:'DG', 3:'BCMP', 4:'FHVWY', 5:'K', 8:'JX', 10:'QZ'}
spisok_ru = { 1:'АВЕИНОРСТ', 2:'ДКЛМПУ', 3:'БГЁЬЯ', 4:'ЙЫ', 5:'ЖЗХЦЧ', 8:'ШЭЮ', 10:'ФЩЪ'}
word = input().upper()
count = 0
for i in word:
    if i in 'AEIOULNSTRDGBCMPFHVWYKJXQZ':
        for key, value in spisok_en.items():             
            if i in value:                
                count = count + key 

    else:
        for key, value in spisok_ru.items():
            if i in value:
                count = count + key
